...
def VID(Controller, Vehicle, rule, routRule, Parameter):
    ...
    if Controller.job_num > 0:
        ...
        return change, Vehicle.ID, jID.ID, Vehicle.path, Vehicle.nextpath
    else:
        return change, Vehicle.ID, 0, 0, 0
...
def WID(Controller, Job, rule, routRule, Parameter):
    ...
    if Controller.IV_num > 0:
        ...
        #LU
            ...
            index = 0
            for i in range(len(candidate)):
                UT = Controller.Time - candidate[i].IdleTime
                if UT < minUT:
                    minUT = UT
                    index = i
        else:
        ...
        vID = candidate.pop(index)
        Controller.jobs.remove(Job)
        Controller.job_num -= 1
        
        return change, vID.ID, Job.ID, vID.path, vID.nextpath
    else:
        return change, 0, Job.ID, 0, 0
